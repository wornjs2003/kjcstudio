#!/usr/bin/env python3
"""뉴스 주제어가 두 곳에서 같은지 대조한다.

    holdings/data/news-topics.json    로컬 서버가 읽는다 (server/news.py)
    holdings/worker/kis-worker.js     배포본이 쓴다 (NEWS_TOPICS 상수)

워커는 대시보드에 코드만 붙여넣는 방식이라 JSON 파일을 같이 올릴 수 없다.
합칠 수 없는 복제다. 한쪽만 고치면 로컬에서는 새 주제가 보이는데 사이트에는
안 보인다. 둘 다 오류를 내지 않아 눈으로는 못 찾는다.

    python tools/check-news-topics.py     맞으면 0, 어긋나면 1

CLAUDE.md 「같은 값은 한 곳에만 둔다」 참조.
"""
import io
import json
import os
import re
import sys

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
JSON_PATH = "holdings/data/news-topics.json"
JS_PATH = "holdings/worker/kis-worker.js"


def read_json():
    path = os.path.join(ROOT, JSON_PATH)
    if not os.path.exists(path):
        return None, "파일이 없습니다: " + JSON_PATH
    raw = json.load(io.open(path, encoding="utf-8"))
    topics = [t for t in (raw.get("topics") or []) if t.get("on", True)]
    drop = ((raw.get("버릴제목") or {}).get("말")) or []
    return {"topics": topics, "drop": drop}, None


def read_js():
    """워커에서 NEWS_TOPICS 와 NEWS_DROP 을 읽는다.

    자바스크립트를 실행하지 않고 글자만 본다. 배열 리터럴이라
    따옴표를 큰따옴표로 바꾸면 JSON 으로 읽힌다."""
    path = os.path.join(ROOT, JS_PATH)
    if not os.path.exists(path):
        return None, "파일이 없습니다: " + JS_PATH
    s = io.open(path, encoding="utf-8").read()

    topics = []
    m = re.search(r"const NEWS_TOPICS = \[(.*?)\n\];", s, re.S)
    if not m:
        return None, "NEWS_TOPICS 를 찾지 못했습니다"
    for block in re.findall(r"\{(.*?)\}", m.group(1), re.S):
        item = {}
        for key in ("id", "color", "label"):
            hit = re.search(key + r'\s*:\s*"([^"]*)"', block)
            if hit:
                item[key] = hit.group(1)
        kw = re.search(r"keywords\s*:\s*\[(.*?)\]", block, re.S)
        item["keywords"] = re.findall(r'"([^"]*)"', kw.group(1)) if kw else []
        on = re.search(r"on\s*:\s*(true|false)", block)
        if on and on.group(1) == "false":
            continue                      # 꺼둔 것은 JSON 쪽에서도 빠진다
        topics.append(item)

    d = re.search(r"const NEWS_DROP = \[(.*?)\];", s, re.S)
    drop = re.findall(r'"([^"]*)"', d.group(1)) if d else []
    return {"topics": topics, "drop": drop}, None


def main():
    a, err = read_json()
    if err:
        print("[읽기 실패] " + err)
        return 1
    b, err = read_js()
    if err:
        print("[읽기 실패] " + err)
        return 1

    print("  news-topics.json   주제 %d · 버릴말 %d" % (len(a["topics"]), len(a["drop"])))
    print("  kis-worker.js      주제 %d · 버릴말 %d" % (len(b["topics"]), len(b["drop"])))

    bad = []

    ida = [t.get("id") for t in a["topics"]]
    idb = [t.get("id") for t in b["topics"]]
    if ida != idb:
        bad.append("주제 목록이 다릅니다\n    json  : %s\n    worker: %s"
                   % (", ".join(map(str, ida)), ", ".join(map(str, idb))))
    else:
        for x, y in zip(a["topics"], b["topics"]):
            for key in ("label", "color"):
                if x.get(key) != y.get(key):
                    bad.append("%s 의 %s 가 다릅니다 — json %r · worker %r"
                               % (x.get("id"), key, x.get(key), y.get(key)))
            if x.get("keywords") != y.get("keywords"):
                bad.append("%s 의 주제어가 다릅니다\n    json  : %s\n    worker: %s"
                           % (x.get("id"), x.get("keywords"), y.get("keywords")))

    if a["drop"] != b["drop"]:
        bad.append("버릴 제목 목록이 다릅니다\n    json  : %s\n    worker: %s"
                   % (a["drop"], b["drop"]))

    print()
    if bad:
        for line in bad:
            print("어긋남  " + line)
        print()
        print("뉴스 주제어: 어긋났습니다. 두 곳을 같게 맞춰 주세요.")
        print("워커를 고쳤으면 Cloudflare 에 다시 올려야 합니다.")
        return 1

    print("뉴스 주제어: 주제 %d개, 두 곳이 같습니다." % len(a["topics"]))
    return 0


if __name__ == "__main__":
    sys.exit(main())
