        ...
        for page in PAGES:
            ...
            if stable < SETTLE_SAME:
                ...
                if miss:
                    ...
                    # **기다림으로 풀리지 않는 것은 실패가 아니다 (2026-09-22).**
                    # 깊이 밖이든 진짜 없어진 칸이든 **다시 붙어도 같은 결과**다.
                    # 전에는 `gone` 이 하나라도 있으면 「못 쟀습니다」(코드 2)로
                    # 보내 **세 번 다시 붙고 끝났다** — 기준이 낡았을 뿐인데
                    # 도구가 아예 안 도는 것처럼 보였다.
                    #
                    # **「못 쟀다」 는 연결이 끊겼을 때만이다.** 칸이 안 나온 것은
                    # 값을 그대로 쓰고 **대조에서 「이번엔 안 나왔습니다」 로 알린다.**
                    result[page] = cells
                    continue
                else:
