...
- 메인페이지(`index.html`)는 **풀스크린 인트로 + 카테고리 입구 목록** (2026-09-12 변경)
  - 첫 화면을 스튜디오 이름으로 꽉 채우고, 스크롤하면 카테고리 4개가 거대 타이포로 나온다
  - 작업물 갤러리는 메인이 아니라 각 `category/*.html` 에 있다
  - 이전에는 메인이 3D Modeling 갤러리를 그대로 띄워서 `category/3d-modeling.html` 과 내용이 겹쳤다
