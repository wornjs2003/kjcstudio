...
### (3) 지수 당일 흐름 — 붙임 (2026-09-14)

첫 화면의 큰 차트가 쓰는 `/api/kis/index-minutes` 를 넣었습니다.

**알아둘 값** — `FID_INPUT_HOUR_1` 은 시각이 아니라 **초 단위 간격**입니다.
`"150000"`(15시)을 넣으면 날짜별 요약이 오고, `"300"`(5분)을 넣으면
09:00~15:30 하루치가 99건으로 옵니다. 공식 문서에 값이 적혀 있지 않아
직접 여러 값을 넣어 찾았습니다.

로컬(`server/kis_proxy.py` 의 `fetch_index_minutes`)과 **같은 값**을 써야
로컬과 배포본의 차트가 같아집니다.

### (4) 목록 시세 — 붙임 (2026-09-14)

첫 화면 순위표가 쓰는 `/api/kis/quotes` 를 넣었습니다.

**놓쳐서 배포본이 깨졌던 건입니다.** f248d77 에서 화면을 이 경로로 바꾸면서
로컬 서버(`server/kis_proxy.py`)에만 넣고 Worker 에 안 넣었습니다. 배포 사이트에서
404 가 나면서 현재가·전일대비가 하나도 안 찍혔습니다.

**교훈** — 화면이 부르는 경로를 새로 만들면 **로컬과 Worker 양쪽에 같이** 넣어야
합니다. 한쪽만 있으면 로컬에서는 멀쩡해 보이고 배포본만 조용히 빕니다.

**알아둘 값**
- `tr_id` `FHKST11300006` · 한 번에 **30종목**까지 (40개를 보내면 앞 30개만 옵니다)
- 응답 항목이 `server/kis_proxy.py` 의 `fetch_quotes_multi` 와 **같아야 합니다.**
  화면은 같은 코드로 로컬과 배포본을 함께 읽습니다
  (`price` `prev` `amt` `pct` `name` `open` `high` `low` `volume` `value` `source`)
- 시가총액·PER·PBR·52주 최고저는 **오지 않습니다.** 그 값이 필요한 종목 화면은
  단건 조회(`/api/kis/prices`)를 그대로 씁니다
