...
/* ── 호출 간격 ──────────────────────────────────────────────
   ...
   ── **기준은 하나, 지키는 방법이 다르다** (2026-09-23) ──────────

   **기준** — KIS 계정 하나가 **초당 20건**(실전 기본)을 안 넘는다.
   그 하나를 이렇게 나눠 쓴다.

       **여기(워커)**   **5건/초**    `KIS_MIN_INTERVAL_MS = 200`
       로컬 여섯       **합쳐서 10건/초**   `kis_proxy.py` 의 공유 줄
       ─────────────────────────────────
       합              **15 / 20**   여유 **25%**

   **로컬은 0.1초이고 여기는 0.2초인데, 값이 어긋난 것이 아니다.**
   로컬은 **서버 여섯이 그 하나를 나눠 쓰고**(파일 락으로 한 줄에 선다),
   여기는 **워커 하나가 제 몫을 쓴다.** 몫이 다르니 간격도 다르다.

   ⚠️ **2026-09-23 에 둘을 0.2 로 맞췄다가 되돌렸다.** 「둘이 갈렸다」 로
   올렸는데, 로컬의 0.1 은 **안 되돌린 값이 아니라 공유 줄이 합계로
   바꿔 놓은 값**이었다. **같은 숫자가 공유 줄 전후로 뜻이 반대다.**

   **여기 값(200)은 그때도 지금도 바뀐 적이 없다.**

   **파일 락은 여기서 못 쓴다** — 다른 기계라 그 파일을 못 본다.
   **「못 합친다」 와 「다르게 정한다」 는 다르다.** 못 합치는 것은
   사실이고, **기준(20 안)은 하나로 둔다.**

   ⚠️ **합계를 바꿀 때는 양쪽을 같이 본다.** 한쪽만 올리면 합이 20 을 넘는다.

   ── 지난 이야기 ────────────────────────────────────────────

   전에는 로컬이 1초라
   「여기만 다르게 잡는다」 고 적어 두었는데, 2026-09-14 에 그쪽
   `BUDGET_RATIO` 를 0.1 → 0.5 로 올리면서 **둘이 같아졌다.**
   그때 이 주석을 안 고쳐 **아홉 날 동안 「다르다」 고 적혀 있었다**
   (2026-09-23 에 고쳤다 · `주식페이지_개발1` 이 사용량을 재다 찾았다).

   **값이 아니라 설명이 낡은 자리라 화면으로는 안 드러난다.**
   `KIS_MIN_INTERVAL_MS` 와 `kis_proxy.py` 의
   `KIS_LIMIT_PER_SEC × BUDGET_RATIO` 가 **같은 값이어야 한다.**
