   ...
   **기준** — KIS 계정 하나가 **초당 20건**(실전 기본)을 안 넘는다.
   그 하나를 이렇게 나눠 쓴다.
       ...
       **여기(워커)**   **5건/초**    `KIS_MIN_INTERVAL_MS = 200`
       로컬 여섯       **합쳐서 5건/초**   `kis_proxy.py` 의 공유 줄
       ─────────────────────────────────
       합              **10 / 20**   여유 **50%**

   **2026-09-23 에 둘을 0.2 로 맞췄다** (재권님 「가로 해놓고」).

   ⚠️ **숫자가 같아졌다고 몫이 같아진 것이 아니다.**
   로컬 0.2 는 **서버 여섯이 나눠 쓰는 5건**이고(파일 락으로 한 줄에 선다),
   여기 0.2 는 **워커 하나가 쓰는 5건**이다. **몫은 여전히 다르다.**
