...
/* ── 호출 간격 ──────────────────────────────────────────────
   ...
   200ms = 초당 5건. 기본 한도의 1/4 이다.

   ⚠️ **2026-09-23 에 로컬이 0.1초로 내려가 값이 갈렸다.** 여기는 200ms
   그대로다 — 로컬은 서버가 여섯인데 여기는 워커 하나라 사정이 다르고,
   바꾸면 대시보드 배포가 따라온다. **맞출지는 재권님이 정하신다.**

   **아래는 그 전 이야기다.** 로컬도 같은 0.2초였다. 전에는 로컬이 1초라
