...
#
# **배포본과 값이 같다.** `worker/kis-worker.js` 의 `KIS_MIN_INTERVAL_MS`
# 도 **200ms** 다.
#
# **하루 안에 갈렸다 다시 같아졌다.** 아침에 여기만 0.1 로 내려
# **어느 쪽을 보는지 갈렸고**, 그날 **재권님이 맞추기로 정하셨다**
# (「가로 해놓고」).
#
# 세 길을 값과 함께 올려 재권님이 고르셨다.
#
#     (가) 둘 다 0.2   로컬 5 + 워커 5 = **10/20** · 여유 **50%** · 배포 불필요
#     (나) 둘 다 0.1   로컬 10 + 워커 10 = 20/20 · **여유 0** · 배포 필요
#     (다) 그대로      로컬 10 + 워커 5 = 15/20 · 여유 25%
#
# **(가)로 정해졌다.** **차트가 느려지는 것을 아시고 고르신 것**이다 —
# 혼자 바쁠 때 **1.90초 → 3.80초**(실측). **공유 줄은 남지만 그것으로
# 빨라진 만큼이 되돌아간다.**
#
# ⚠️ **「간격을 맞춘다」 와 「몫을 맞춘다」 는 다르다.**
# 여기 0.2 는 **여섯이 나눠 쓰는 5건**이고, 워커 0.2 는 **혼자 쓰는 5건**이다.
# **같은 숫자인데 몫이 다르다.**
# **`tools/check-kis-consts.py` 는 이 값을 안 본다** — 재서 확인했다.
#
# 이 두 값만 바꾸면 호출량 전체가 조절된다.
KIS_LIMIT_PER_SEC = 10.0
BUDGET_RATIO = 0.5                                  # 기준값의 1/2
# ⚠️ **이 값은 서버 하나가 아니라 여섯이 합쳐서 쓰는 몫이다** (2026-09-23).
# 아래 공유 줄이 여섯을 한 줄에 세우므로, 고정으로 나눌 때처럼
# 「각자 N건 × 여섯」 이 되지 않는다. **합쳐서 5건이다.**
#
# 배포본 워커는 이 줄 **밖**이라 따로 초당 5건을 쓴다 —
# 합하면 **10 / 20** 으로 실제 한도에 **50% 여유**가 남는다.
KIS_CALLS_PER_SEC = KIS_LIMIT_PER_SEC * BUDGET_RATIO   # = **합쳐서** 초당 5건
