...
# 지금 쓰는 양은 실제 한도의 50% 수준이다. 자세한 내용은 docs/data-sources.md 참고.
#
...
#     2026-09-23   0.5 → **1.0**   초당 5 → **10건**   ← 재권님 지시 「0.1초로 해줘」
#
...
#
# ⚠️ **배포본과 값이 갈렸다.** `worker/kis-worker.js` 의
# `KIS_MIN_INTERVAL_MS` 는 **200ms(초당 5건) 그대로**다. 로컬은 서버가 여섯인데
# 배포본은 워커 하나라 사정이 다르고, **바꾸면 대시보드 배포가 따라온다.**
# 둘을 맞출지는 재권님이 정하신다 (2026-09-23 여쭙는 중).
# **`tools/check-kis-consts.py` 는 이 값을 안 본다** — 재서 확인했다.
#
# 이 두 값만 바꾸면 호출량 전체가 조절된다.
KIS_LIMIT_PER_SEC = 10.0
BUDGET_RATIO = 1.0                                  # 기준값을 다 쓴다
KIS_CALLS_PER_SEC = KIS_LIMIT_PER_SEC * BUDGET_RATIO   # = 초당 10건
