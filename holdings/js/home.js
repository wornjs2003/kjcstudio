...
startLiveLoop({
  ...
  /* !! 임시 3000 — 고쳐지면 1000 으로 되돌릴 것 (2026-09-15 지시)
     1초로 두니 배포본에서 지수가 한 번 갱신되고 멈췄다.
     /api/kis/indices 한 번이 KIS 를 11번 부르는데(국내 현재값 3 + 추이 3 +
     해외 4 + 선물 1), 워커가 KIS_MIN_INTERVAL_MS 200ms 로 간격을 강제해
     한 번에 최소 2.2초가 걸린다. 로컬 실측도 2.27초였다.
     2.2초 걸리는 일을 1초마다 시키니 kisPace 의 대기 줄이 밀려 멈췄다.

     되돌리는 조건 — 아래 둘이 끝나면 1000 으로 돌린다.
       1. fetchIndexSeries(작은 꺾은선)를 D1 에서 읽기.
          60일치 과거 봉은 하루에 한 번 바뀌는데 매초 다시 받고 있다.
          큰 차트(index-candles)는 이미 D1 에 쌓아놓고 읽는다. 같은 일봉이다
       2. 해외 4개를 지수 호출의 줄에서 빼기 (OVERSEAS_TTL 60 인데 같은 줄에
          서 있어 만료될 때마다 1.6초 스파이크를 만든다)
     둘이 끝나면 1초마다 4번(현재값 3 + 선물 1)이라 0.8초로 감당된다. */
  indexMs: 3000,
